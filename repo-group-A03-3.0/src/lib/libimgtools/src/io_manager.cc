...
PixelBuffer* IOManager::LoadImageToCanvas(void) {
  ...
  file_io = factory_->createFileIO(file_name_);
  if (file_io != nullptr) {
    return file_io->readFile(file_name_);
  } else {
    return nullptr;
  }
}

PixelBuffer* IOManager::LoadImageToCanvas(const std::string& file_name) {
  if (is_valid_image_file(file_name)) {
    FileIO* file_io = nullptr;
    file_io = factory_->createFileIO(file_name);
    if (file_io != nullptr) {
      return file_io->readFile(file_name);
    } else {
      return nullptr;
    }
  } else {
...
PixelBuffer* IOManager::LoadImageToStamp(void) {
  /* LoadImageToStamp and LoadImageToCanvas are Identical */
  return LoadImageToCanvas();
}

PixelBuffer* IOManager::LoadImageToStamp(const std::string& file_name) {
  return LoadImageToCanvas(file_name);
}

void IOManager::SaveCanvasToFile(const PixelBuffer* curr_canvas) {
  FileIO* file_io = nullptr;
  file_io = factory_->createFileIO(file_name_);
  if (file_io != nullptr) {
    file_io->saveToFile(file_name_, curr_canvas);
  } else {
    fprintf(stderr, "Factory Failed to create FileIO\n");
  }
}
...
void IOManager::SaveCanvasToFile(const PixelBuffer* curr_canvas,
  ...
  file_io = factory_->createFileIO(file_name);
  if (file_io != nullptr) {
   file_io->saveToFile(file_name, curr_canvas);
  } else {
   fprintf(stderr, "Factory Failed to create FileIO\n");
  }
}
