...
BlurTool::BlurTool(int width, int height)
    : Tool(width, height) {
}
...
void BlurTool::paint(int x, int y, PixelBuffer* canvas, ColorData curr_color,
                     int opa_lvl) {
  ...
  for (int i = 0; i < getMaskWidth(); i++) {
    for (int j = 0; j < getMaskHeight(); j++) {
      ...
      if ((cor_x >= 0) && (cor_x < canvas->width()) && (cor_y >= 0)
          && (cor_y < canvas->height())) {
        float red = 0.0;
        float green = 0.0;
        float blue = 0.0;
        ...
        for (int kernelY = 0; kernelY < kernel_height; kernelY++) {
          for (int kernelX = 0; kernelX < kernel_width; kernelX++) {
            ...
            if (bufferX >= 0 && bufferX < canvas->width() && bufferY >= 0
                && bufferY < canvas->height()) {
