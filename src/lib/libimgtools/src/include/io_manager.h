...
class IOManager {
 public:
  ...
  inline void setIOFactory(FileIOFactory* factory) {
    factory_ = factory;
  }
   ...
   *
   */
  virtual PixelBuffer* LoadImageToCanvas(void);
   ...
   *
   */
  virtual PixelBuffer* LoadImageToCanvas(const std::string& file_name);
   ...
   *
   */
  PixelBuffer* LoadImageToStamp(const std::string& file_name);

  /**
   * @brief Save the current state of the canvas to a file
   *
   */
  virtual void SaveCanvasToFile(const PixelBuffer* curr_canvas);
